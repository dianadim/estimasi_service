...
class _EstimasiPageState extends State<EstimasiPage> {
  ...
  static const String _kCloudJasaVersion = 'eco_cloud_jasa_version_v1';
  ...
  RealtimeChannel? _masterJasaChannel;
...
{sa} - Service Advisor''';
  ...
  Future<bool> _masterCloudBerubah() async {
    ...
    final versions = await Future.wait([
      _versiMasterServer('eco_master_jasa'),
      _versiMasterServer('eco_master_sparepart'),
      _versiMasterServer('eco_master_bahan'),
    ]);

    final serverJs = versions[0];
    final serverSp = versions[1];
    final serverBh = versions[2];
    final localJs = prefs.getString(_kCloudJasaVersion) ?? '';
    ...
    return serverJs != localJs || serverSp != localSp || serverBh != localBh;
  }
  ...
  Future<void> _sinkronMasterCloud({
    ...
    try {
      ...
      final hasil = await Future.wait([
        _ambilSemuaBaris(
          'eco_master_jasa',
          'nama_jasa,tipe_kendaraan,harga',
        ),
        _ambilSemuaBaris(
      ...
      // Migrasi satu kali: saat tabel cloud baru masih kosong, kirim Master
      // Jasa yang sudah tersimpan di device ini agar data lama tidak hilang.
      if (jsRaw.isEmpty && jasaDatabase.isNotEmpty) {
        for (final jasa in jasaDatabase) {
          await Supabase.instance.client.from('eco_master_jasa').upsert({
            'nama_jasa': jasa.nama.trim(),
            'tipe_kendaraan': jasa.tipeKendaraan.trim(),
            'harga': jasa.harga,
            'updated_at': DateTime.now().toUtc().toIso8601String(),
          }, onConflict: 'nama_jasa,tipe_kendaraan');
        }
        jsRaw = await _ambilSemuaBaris(
          'eco_master_jasa',
          'nama_jasa,tipe_kendaraan,harga',
        );
      }
      ...
      final prefs = await SharedPreferences.getInstance();
      await prefs.setString(_kCloudJasaVersion, versions[0]);
      await prefs.setString(_kCloudSparePartVersion, versions[1]);
      await prefs.setString(_kCloudBahanVersion, versions[2]);

      // Master Jasa tetap disimpan lokal agar pencarian cepat dan tetap
      // berfungsi ketika offline, tetapi sumber bersama berasal dari cloud.
      final jasaCloud = jsRaw.map((e) => DatabaseItem(
        kode: '',
        nama: (e['nama_jasa'] ?? '').toString().trim(),
        tipeKendaraan: (e['tipe_kendaraan'] ?? '').toString().trim(),
        harga: (e['harga'] as num?)?.toInt() ?? 0,
      )).where((e) => e.nama.isNotEmpty).toList();
      if (mounted) {
        setState(() {
          jasaDatabase
            ..clear()
  ...
  void _dengarkanMasterJasaRealtime() {
    if (_masterJasaChannel != null) {
      unawaited(Supabase.instance.client.removeChannel(_masterJasaChannel!));
    }
    _masterJasaChannel = Supabase.instance.client
        .channel('eco-master-jasa-${DateTime.now().millisecondsSinceEpoch}')
        .onPostgresChanges(
          event: PostgresChangeEvent.all,
          schema: 'public',
          table: 'eco_master_jasa',
          callback: (_) {
            // Paksa agar perubahan dari device lain langsung masuk ke cache lokal.
            unawaited(_sinkronMasterCloud(paksa: true));
          },
        )
        .subscribe();
  }
  ...
  Future<void> _simpanItemMasterCloud(int tab, DatabaseItem? lama, DatabaseItem baru) async {
    ...
    if (tab == 0) {
      if (lama != null &&
          (lama.nama.trim().toLowerCase() != baru.nama.trim().toLowerCase() ||
           lama.tipeKendaraan.trim().toLowerCase() !=
               baru.tipeKendaraan.trim().toLowerCase())) {
        await client
            .from('eco_master_jasa')
            .delete()
            .eq('nama_jasa', lama.nama)
            .eq('tipe_kendaraan', lama.tipeKendaraan);
      }
      await client.from('eco_master_jasa').upsert({
        'nama_jasa': baru.nama,
  ...
  Future<void> _hapusItemMasterCloud(int tab, DatabaseItem item) async {
    ...
    if (tab == 0) {
      await client
          .from('eco_master_jasa')
          .delete()
          .eq('nama_jasa', item.nama)
          .eq('tipe_kendaraan', item.tipeKendaraan);
    } else if (tab == 1) {
          ...
          actions: [
            ...
            FilledButton(
              onPressed: () async {
                ...
                final lama = indexMaster >= 0 ? jasaDatabase[indexMaster] : null;
                final jasaBaru = DatabaseItem(
                  kode: '',
                  nama: nama,
                  harga: harga,
                  tipeKendaraan: tipe,
                ...
                try {
                  await _simpanItemMasterCloud(0, lama, jasaBaru);
                } catch (err) {
                  if (!mounted) return;
                  ScaffoldMessenger.of(context).showSnackBar(
                    SnackBar(content: Text('Gagal menyimpan Master Jasa: $err')),
                  );
                  return;
                }
                if (indexMaster >= 0) {
  ...
  Future<void> bukaEditDatabase([int awal = 0]) async {
    // Ambil perubahan terbaru sebelum daftar master ditampilkan.
    await _sinkronMasterCloud(paksa: true);
  ...
  void _dengarkanTemuanRealtime() {
    _ecoFindingsChannel = Supabase.instance.client
        ...
        .onPostgresChanges(
          ...
          schema: 'public',
          table: 'eco_findings',
          callback: (_) => unawaited(_sinkronTemuanCloud()),
        )
        .subscribe();
  }
      ...
      builder: (dc) => StatefulBuilder(
        builder: (context, setD) {
          return AlertDialog(
            ...
            title: Row(
              children: [
                Expanded(child: Text((data['noPolisi'] ?? 'Detail Temuan').toString())),
